...
for quiz_num in range(issue):
    #問題集と回答集のファイルを作成
    quiz_file = open(f"capitalquiz{quiz_num}.txt", "w")
    ...
    for question_num in range(len(prefectures)):
        ...
        answer_options = wrong_answers + [correct_answer] #不正解と正解の選択肢を合わせる(４択になる)
        ...
        for i in range(4): #４択の選択肢を出力
            quiz_file.write(f' {"ABCD"[i]}. {answer_options[i]}\n') #選択肢一つずつ
        quiz_file.write("\n") #問題文の区切りの空行

        #答えの選択肢を解答ファイルに出力する
        answer_key_file.write(f"{question_num + 1}. ") #問題番号
        answer_key_file.write("ABCD"[answer_options.index(correct_answer) ] ) #正解インデックスの番号の選択肢だけ
